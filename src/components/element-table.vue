  ...
  <div class="lr-table">
    ...
    <div class="lr-whole-top">
      <div v-if="$slots.search && showClear" class="lr-table--action--clear">
        <i class="iconfont icon-ic_refresh_normal" @click="clear" /><span @click.stop="clear" class="lr-font12">清空筛选条件</span>
      </div>
      ...
      <div>
        <slot />
        <div v-if="needPagination" class="lr-table--pagination">
  ...
  methods: {
    ...
    loadPage() {
      if (this.url) { // 加载数据
        ...
        const needResetPageNumber = this.isResetPageNumber(this.page.getPageModel(), this.getSearchModel())
        const searchModel = Object.assign(this.page.getPageModel(needResetPageNumber ? 1 : null), this.getSearchModel())
        // 更新搜索历史
